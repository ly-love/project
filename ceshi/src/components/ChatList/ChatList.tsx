...
import { db, Message, Article, majors, Major } from "../../utils/db";
...
function ChatList({ chatData = [] }: ChatListProps) {
  ...
  const getMajorTable = (type: "Messages" | "Articles") => {
    return db.table<Message | Article>(`${marjor}${type}`);
  };

  const fetchMessages = async () => {
    const subjectTable = getMajorTable("Messages");
    const allMessages = await subjectTable.toArray();
  ...
  useEffect(() => {
    scrollToBottom();
  }, [messages, chatData, scrollToBottom]); // item.from === "mine"滚动条未滚动，所以增加 chatData更新触发scrollToBottom，确保滚动条移动到最新内容的位置
